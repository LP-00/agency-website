...
for(let index=0;index<sections.length;index++){
  ...
  const resized=await sharp(`qa/${pass}/390-${name}.png`).resize({width,height:height-28,fit:"inside"}).toBuffer();
...
await sharp({create:{width:columns*(width+gap)+gap,height:Math.ceil(sections.length/columns)*(height+gap)+gap,channels:3,background:"#0a0b0b"}}).composite(layers).webp({quality:88}).toFile(`qa/${pass}/contact-sheet-mobile.webp`);
